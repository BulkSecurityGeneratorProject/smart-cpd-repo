...
public interface ChoiceRepository extends JpaRepository<Choice, Long>, JpaSpecificationExecutor<Choice> {
    ...
    @Query(value = "SELECT TOP 1 * FROM COURSE_HISTORY course_history WHERE course_history.customer.id = :customer_id ORDER BY course_history.id DESC", nativeQuery = true)
