...
public class CartResource {
    ...
    public String chargeCard(HttpServletRequest request, @RequestBody Cart cart) throws Exception {
        ...
        Cart tempCart = this.cartService.findOne(cartId).orElse(null);
        Charge temp = this.stripeClient.chargeNewCard(token, ((amount - redeem)*100));
        ...
        if((temp.getStatus().equals("succeeded"))&&temp.getPaid()) {
            ...
            newOrder.setAmount(tempCart.getAmount()/100);
        ...
        return /*(newOrder.getId().toString()) + */temp.toJson();
    }
