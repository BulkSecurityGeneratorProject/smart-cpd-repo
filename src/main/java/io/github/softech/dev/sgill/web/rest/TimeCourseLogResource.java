...
public class TimeCourseLogResource {
    ...
    @GetMapping("{courseid}/get/time-course-logs/{customerid}")
    @Timed
    public ResponseEntity<TimeCourseLog> getDistinctTimeCourseLogs(@PathVariable Long courseid, @PathVariable Long customerid) {
        log.debug("REST request to get Time Spent by Course ID : {}", courseid);
        log.debug("REST request to get Time Spent by Customer ID : {}", customerid);
        return ResponseUtil.wrapOrNotFound(timeCourseLogRepository.findTimeCourseLogByCustomerIdAndCourseId(customerid, courseid));
    }
